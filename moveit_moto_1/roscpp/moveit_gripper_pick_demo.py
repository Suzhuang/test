...
class MoveItDemo:
	...
	def __init__(self):
		...
		#left gripper
		l_p=PoseStamped()
		l_p.header.frame_id=gripper_end_effector_link
		l_p.pose.position.x=0.00
		l_p.pose.position.y=0.04
		l_p.pose.position.z=0.04
		l_p.pose.orientation.w=1
		scene.attach_box(gripper_end_effector_link,'l_tool',l_p,l_tool_size)	
		
		#right gripper
		r_p=PoseStamped()
		r_p.header.frame_id=gripper_end_effector_link
		r_p.pose.position.x=0.00
		r_p.pose.position.y=-0.04
		r_p.pose.position.z=0.04
		r_p.pose.orientation.w=1
		scene.attach_box(gripper_end_effector_link,'r_tool',r_p,r_tool_size)	
		
		#grasp
		g_p=PoseStamped()
		g_p.header.frame_id=gripper_end_effector_link
		...
		gripper.set_pose_target(grasp_pose,gripper_end_effector_link)
		traj=gripper.plan()
		...
		'''
		gripper.shift_pose_target(0,0.11,gripper_end_effector_link)
		gripper.go()
		rospy.sleep(2)
		saved_target_pose=gripper.get_current_pose(end_effector_link)
		...
		scene.remove_attached_object(gripper_end_effector_link,'l_tool')
		scene.remove_attached_object(gripper_end_effector_link,'r_tool')
